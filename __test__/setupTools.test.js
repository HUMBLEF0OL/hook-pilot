...
describe('setupTools', () => {
    ...
    it('should install and initialize Husky if not already set up', () => {
        ...
        expect(execSync).toHaveBeenNthCalledWith(1, 'npm install husky --save-dev', { stdio: 'inherit' });

        // Verify execSync is called to initialize Husky
        expect(execSync).toHaveBeenNthCalledWith(2, 'npx husky init', { stdio: 'inherit' });

        // Verify logs
        expect(console.log).toHaveBeenCalledWith(expect.stringContaining('🔧 Setting up Husky'));
        expect(console.log).toHaveBeenCalledWith(expect.stringContaining('✅ Husky setup complete!'));
    });
    ...
    it('should skip Husky setup if already initialized', () => {
        ...
        expect(execSync).not.toHaveBeenCalledWith('npm install husky --save-dev', { stdio: 'inherit' });
        expect(execSync).not.toHaveBeenCalledWith('npx husky init', { stdio: 'inherit' });
