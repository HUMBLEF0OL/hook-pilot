...
const selectTemplate = async (hookType) => {
    try {
        ...
        // Add 'Custom' option for creating custom templates
        ...
        // Map back selected templates to their full details
        const selectedTemplates = templates.map(templateValue => {
            return choicesWithCustom.find(choice => choice.name === templateValue.split(":")[0]);
        });

        // Display feedback for templates requiring configuration
        const templatesNeedingConfig = selectedTemplates.filter(t => t.requiresConfig);

        if (templatesNeedingConfig.length > 0) {
            console.warn(`⚙️ The following templates require additional configuration:`);
